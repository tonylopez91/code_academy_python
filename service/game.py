...
from model.weapons import Physical, Magical 3
...
class Game:
  ...
  def prompt_set_missions(self):
    while self.__are_missions_left != False:
      ...
      mission_monster = [monster for monster in self.get_monsters() if monster.get_type() == mission['monster']].pop()    
      ...
      while is_hero_alive and is_monster_alive:
        ...
        if(action['purpose'] == configs.hero_actions['Escape']):
          escape = [purpose for purpose in action['actions'] if purpose['result'] == action['purpose'] and purpose['damaged'] == has_been_battle].pop()
  ...
  def hero_fight_monster(self, monster_to_fight: Monster):
    hero = self.get_hero()
    self.dramatical_print(f'{hero.get_name()} will strike {monster_to_fight.get_name()} with your mighty weapon {hero.get_weapon().get_name()}')

    (damage, material) = hero.attack()
    monster_to_fight.recieve_attack(damage, material)
    self.dramatical_print(f'You manage to dealt {damage} points to {monster_to_fight.get_name()} now its life drops to: {monster_to_fight.get_life()} ')
  ...
  def monster_fight_hero(self, monster: Monster):
    ...
    self.dramatical_print(f'{monster.get_name()} has strike you down with {damage} points now your life drops to: {hero.get_life()} ')